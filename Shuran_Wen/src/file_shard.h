...
inline bool shard_files(const MapReduceSpec& mr_spec, std::vector<FileShard>& fileShards) {
	...
	for (int i = 0; i < num_files; i++) {
          ...
          int size = (statbuf.st_size/1024) + 1;  
          file_size[i] = size; //to get all file content
          // std::cout << "File: " << i << " Size: " << size << std::endl;
          sum_size += size;
	}
     ...
     for (int i = 0; i < num_shards; i++) {
          ...
          while(current_map_remain > 0){
               ...
               if((current_file_end - current_file_start) >= current_map_remain){
                    ...
                    // std::cout << "File Shard: " << i << " Filename: " << info.filename << " start: " << info.start << " end: " << info.end << std::endl;
